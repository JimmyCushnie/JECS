...
SUCC is a tool for building game configuration files. The API is is simple and easy to use, and the file format it uses is easy for humans to read, write and modify. The SUCC file format is based on [PENIS](https://github.com/JimmyCushnie/PENIS), and it has all the lovely formatting options that PENIS does; however, SUCC introduces some [fancy tools](https://github.com/JimmyCushnie/SUCC#shortcuts) specific to C# which make writing config files even nicer. This makes the SUCC language a superset of PENIS.
